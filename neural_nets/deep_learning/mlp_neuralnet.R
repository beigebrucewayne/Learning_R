## build multilayer neural net
## capable to predict whether school is public or private
## based on other 17 variables
...
deep_net <- neuralnet(f, data = train_data, hidden = c(5, 5, 3, 3), linear.output = F)
plotnet(deep_net)

predicted_data <- compute(deep_net, test_data[,2:18])
print(head(predicted_data$net.result))

# round to 1 digit to compare against original labeling :: 1 vs 0
predicted_data$net.result <- sapply(predicted_data$net.result, round, digits = 0)

t <- table(test_data$Private, predicted_data$net.result)
t
(acc <- (t[1] + t[2,2]) / (t[1] + t[2,2] + t[1,2] + t[2, 1]))
